...
namespace Rami.Wechat.Core.Merchant.Public
{
    ...
    public class WxPayApi
    {
        ...
        /// 公众号配置
        /// </summary>
        internal static PublicConf PubConf { get; set; }
        /// <summary>
        /// 公众号支付配置
        /// </summary>
        internal static MerchantConf PubPayConf { get; set; }
        ...
        public static WxPayData OrderQuery(WxPayData inputObj, int timeOut = 6)
        {
            ...
            WechatHelper.Log.Debug("WxPayApi:OrderQuery request : " + xml);
            string response = HttpService.Post(xml, url, false, timeOut);//调用HTTP通信接口提交数据
            WechatHelper.Log.Debug("WxPayApi:OrderQuery response : " + response);
        ...
        public static WxPayData Reverse(WxPayData inputObj, int timeOut = 6)
        {
            ...
            var start = DateTime.Now;//请求开始时间
            WechatHelper.Log.Debug("WxPayApi:Reverse request : " + xml);

            string response = HttpService.Post(xml, url, true, timeOut);
            WechatHelper.Log.Debug("WxPayApi:Reverse response : " + response);
        ...
        public static WxPayData RefundQuery(WxPayData inputObj, int timeOut = 6)
        {
            ...
            WechatHelper.Log.Debug("WxPayApi:RefundQuery request : " + xml);
            string response = HttpService.Post(xml, url, false, timeOut);//调用HTTP通信接口以提交数据到API
            WechatHelper.Log.Debug("WxPayApi:RefundQuery response : " + response);
        ...
        public static WxPayData DownloadBill(WxPayData inputObj, int timeOut = 6)
        {
            ...
            WechatHelper.Log.Debug("WxPayApi:DownloadBill request : " + xml);
            string response = HttpService.Post(xml, url, false, timeOut);//调用HTTP通信接口以提交数据到API
            WechatHelper.Log.Debug("WxPayApi:DownloadBill result : " + response);
        ...
        public static WxPayData ShortUrl(WxPayData inputObj, int timeOut = 6)
        {
            ...
            WechatHelper.Log.Debug("WxPayApi:ShortUrl request : " + xml);
            string response = HttpService.Post(xml, url, false, timeOut);
            WechatHelper.Log.Debug("WxPayApi:ShortUrl response : " + response);
        ...
        private static void ReportCostTime(string interface_url, int timeCost, WxPayData inputObj)
        {
            ...
            {
                //不做任何处理
                WechatHelper.Log.Error("ReportCostTime:出错：" + ex.Message + "      " + ex.StackTrace);
            }
        }
        ...
        //{
        //    var ran = new Random();
        //    return string.Format("{0}{1}{2}", PubPayConf.MchId, DateTime.Now.ToString("yyyyMMddHHmmss"), ran.Next(999));
        //}
